...
const WaveSVG = styled.svg.withConfig({
  shouldForwardProp: (prop) => !["windowWidth", "scrollY"].includes(prop),
})`
  width: 3000px !important;
  height: 300px;
  transform: translateX(
    ${(props) => -props.windowWidth + props.scrollY * 0.4}px
  );
...
const SecondWaveSVG = styled.svg.withConfig({
  shouldForwardProp: (prop) => !["windowWidth", "scrollY"].includes(prop),
})`
  width: 3000px !important;
  height: 300px;
  transform: translateX(
    ${(props) => -props.windowWidth + props.scrollY * 0.7}px
  );
  background-color: transparent;
`;

const WavySeparator = ({ color }) => {
