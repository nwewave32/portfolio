...
const WaveSVG = styled.svg
  .attrs((props) => ({
    style: {
      transform: `translateX(${props.scrollY * 0.4 - props.windowWidth}px)`,
    },
  }))
  .withConfig({
...
const SecondWaveSVG = styled.svg
  .attrs((props) => ({
    style: {
      transform: `translateX(${props.scrollY * 0.7 - props.windowWidth}px)`,
    },
  }))
  .withConfig({
    shouldForwardProp: (prop) => !["windowWidth", "scrollY"].includes(prop),
...
const WavySeparator = ({ mainColor, backColor }) => {
