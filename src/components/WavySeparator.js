...
const WaveSVG = styled.svg
  .attrs(({ scrollY, windowWidth, pace }) => ({
    style: {
      // transform: `translateX(${scrollY * pace - windowWidth}px)`,
      transform: `translate3d(${
        windowWidth - 3000 + scrollY * pace
      }px, 0px, 0px)`,
    },
  }))
  .withConfig({
    shouldForwardProp: (prop) =>
      !["windowWidth", "scrollY", "pace"].includes(prop),
  })`
  width: auto;
  height: 300px;
...
const WavySeparator = ({
  ...
  const handleScroll = () => {
    ...
    console.log("##window.outerWidth", window.outerWidth, window.scrollY);
  };
